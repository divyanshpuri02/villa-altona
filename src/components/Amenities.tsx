      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="text-5xl lg:text-6xl font-bold text-white mb-6 font-serif">
            Unparalleled <span className="text-yellow-400">Amenities</span>
          </h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {amenities.map((amenity, index) => (
            <motion.div 
              key={index}
              className="glass-effect p-8 rounded-xl hover:bg-white/10 transition-all duration-500 transform hover:-translate-y-2 border border-white/10 group"
              initial={{ opacity: 0, y: 50 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              <motion.div 
                className="bg-yellow-400/20 w-16 h-16 rounded-full flex items-center justify-center mb-6 mx-auto group-hover:bg-yellow-400/30 transition-colors duration-300"
                whileHover={{ rotate: 360 }}
                transition={{ duration: 0.6 }}
              >
                <amenity.icon className="h-8 w-8 text-yellow-400" />
              </motion.div>
              <h3 className="text-xl font-semibold text-white mb-3 text-center font-serif">
                {amenity.title}
              </h3>
